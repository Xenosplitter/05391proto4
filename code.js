...
function draw()
{
  ...
  if (startTime!=0)
  {
    ...
    text(predChar, width/2-createCanvasOfInputArea/4, height/2+createCanvasOfInputArea/2.5);
    textAlign(CENTER);
...
function singleTap()
{
  ...
  if(didMouseClick(width/2-createCanvasOfInputArea/2, height/2+createCanvasOfInputArea/4, createCanvasOfInputArea, createCanvasOfInputArea/4)) {
    ...
    currentTyped = currentTyped + predChar;
  ...
  if(['a','e','i','o','u','y'].includes(lastChar)) {
      consto = ['r','s','t','p','c','b','m']
      predChar = consto[int(random(0,7))]
  } else {
      vow = ['a','e','i','o','u']
      predChar = vow[int(random(0,5))]
